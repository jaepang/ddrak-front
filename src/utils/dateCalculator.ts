...
export const calcHeaderDate = (cur: Date) => {
...
export const inDay = (start: Date, end: Date) =>
  6 <= start.getHours() && (6 <= end.getHours() || (end.getHours() === 0 && end.getMinutes() === 0))

export const nightTime = (date: Date, isStart: boolean) => {
  if (isStart)
    return date.getHours() < 6 ? date : new Date(date.getFullYear(), date.getMonth(), date.getDate() + 1, 0, 0)
  else return date.getHours() < 6 ? date : new Date(date.getFullYear(), date.getMonth(), date.getDate(), 6, 0)
}

export const isBorrowed = (creator: string, club: string) => creator !== 'admin' && creator.slice(0, -5) !== club

export const isInBoundary = (date: Date, curDate: Date) => {
