...
export default class PageStore {
  ...
  handleLogin = async e => {
    ...
    try {
      const res = await axios.post<Data>('/token-auth/', req)
      if (res.status === 200) {
  ...
  getCurUser = async () => {
    try {
      const res = await axios.get<Data>('/api/current-user/', {
        headers: {
  ...
  setUsertype = () => {
    if (this.isSuper) this.usertype = 'admin'
    else if (this.isAdmin) this.usertype = 'clubAdmin'
    else this.usertype = 'club'
  }

  setUserclub = (username, usertype) => {
    const clubs = ['악의꽃', '막무간애', '모여락']
    if (usertype === 'club') this.userclub = username
    else if (usertype === 'clubAdmin') this.userclub = username.substring(0, username.length - 5)
    if (this.userclub !== 'none') this.otherclubs = clubs.filter(c => c !== this.userclub)
  }
