...
const SetTimeSlot = observer(() => {
  ...
  const changeStartTimeSlot = (time: string) => calendar.changeStartTimeSlot(time)
  const changeEndTimeSlot = (time: string) => calendar.changeEndTimeSlot(time)
  const changeDays = (day: number, club: string) => calendar.changeDays(day, club)
  const changeTitle = (title: React.ChangeEvent<HTMLInputElement>) => calendar.changeTitle(title)
  ...
  const idxOf = (i: string) => gridHeader.indexOf(i)
    ...
    <React.Fragment>
      <Container>
        ...
        <div>
          ...
          />
          <ButtonIcon size="medium" icon={<StyledIcon icon={faChevronRight} />} onClick={nextTimeSlot} />
        </div>
      </Container>
        ...
        <div>
          ...
          <GridContainer>
            {gridHeader.map((e, index) => (
              <GridHeader key={index}>{e.toString()}</GridHeader>
            ))}
            {gridHeader.map((e, index) => (
              <LabelContainer id={e} key={index}>
                ...
                />
                <RadioLabel htmlFor={'악의꽃' + String((idxOf(e) + 1) % 7)}>
                  <span>악</span>
                </RadioLabel>
                <RadioInput
                  type="radio"
                ...
                />
                <RadioLabel htmlFor={'막무간애' + String((idxOf(e) + 1) % 7)}>
                  <span>막</span>
                </RadioLabel>
                <RadioInput
                  type="radio"
                ...
                <RadioLabel
                  className={
                    idxOf(e) === 0 || idxOf(e) === 6 ? (idxOf(e) === 0 ? 'border-left' : 'border-right') : 'null'
                  }
          ...
          <Container>
            <StyledButton label="적용" variant="brand" disabled={calendar.disableSubmitButton} onClick={submitData} />
            <StyledButton label="취소" variant="destructive" onClick={disableSetCalendarMode} />
          </Container>
        </div>
      ) : (
        <div>
          <Container>
            <StyledDateTimePicker
              label="시작 시간"
              value={calendar.setTimeSlot[calendar.setTimeIdx].startTime}
              onChange={(t: any) => changeStartTimeSlot(t)}
            />
            <StyledDateTimePicker
              label="끝 시간"
              value={calendar.setTimeSlot[calendar.setTimeIdx].endTime}
              onChange={(t: any) => changeEndTimeSlot(t)}
            />
          </Container>
          <Container>
            <StyledInput
